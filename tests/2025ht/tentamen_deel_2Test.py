...
@test(10)
def test_flatten_unique (test):
    """1. flatten_unique werkt correct"""
    assert function_defined_in_module("flatten_unique")
    func = get_function("flatten_unique")
    assert no_input_output_in_function(func)
    assert func({}) == []
    assert func({"a": []}) == []
    assert func({"a": [1, 2], "b": [2, 3]}) == [1, 2, 3]
    assert func({"a": [1, 2], "b": [3, 2, 0]}) == [1, 2, 3, 0]

@test(10)
def test_adjacent (test):
    """2. adjacent werkt correct"""
    assert function_defined_in_module("adjacent")
    func = get_function("adjacent")
    assert no_input_output_in_function(func)
    assert func({(0, 4), (2, 3), (0, 0), (3, 2), (3, 0)}) == {0: {4, 0}, 2: {3}, 3: {2, 0}}
    assert func({(1, 2)}) == {1: {2}}
    assert func({(0, 4), (0, 0)}) == {0: {4, 0}}
    assert func(set()) == {}
...
def require_doctests_for_all_functions(test):
    ...
    raise AssertionError(
        build_test_type_overview(
            ...
            class_order=["TaskList", "Kroeg", "Kroegentocht", "RationalNumber"],
